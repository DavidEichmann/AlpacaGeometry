...
-- Vector version
--
newtype ShadowFrontV l p = ShadowFrontV { unShadowFrontV :: Vector (WLine l p) }
    deriving (Show,Generic)

instance (NFData l, NFData p) => NFData (ShadowFrontV l p)

emptyV :: ShadowFrontV l p
emptyV = ShadowFrontV V.empty

toShadowFrontV :: forall l p. (Fractional p, Ord p, Show p, Show l)
    => Pos p
    -> Maybe (Vector (l, Seg p))
    -> Vector (l, Seg p)
    -> ShadowFrontV l p    -- this is O(n log(n)) (???are you sure???) by using divide and conquer (and merge)
toShadowFrontV focalPoint filterLabeledSegsMay labeledSegs
    ...
    $ labeledSegs
    where
        applyFilter :: ShadowFrontV l p -> ShadowFrontV l p
        applyFilter = case filterLabeledSegsMay of
            Nothing   -> id
            Just flts -> error "dont support filter segs in Vector version of Shadow Front (yet)." -- filterMerge focalPoint (filterWLines flts) -- %%%  call toShadowFront for flts instead of what is being done here

        -- %%% see last comment above, this us just a special case of toShadowFront
        filterWLines :: Vector (l, Seg p) -> ShadowFrontV l p
        filterWLines = toShadowFrontV' . V.map toWLinesV

        toShadowFrontV' :: Vector (ShadowFrontV l p) -> ShadowFrontV l p
        toShadowFrontV' fs
            | null fs           = ShadowFrontV V.empty
            | V.length fs == 1    = fs!0
            | otherwise         = mergeV focalPoint (toShadowFrontV' fsA) (toShadowFrontV' fsB)  -- %%% instead of having a toShadowFrontV', can you do this at the top level?
            where
                (fsA, fsB) = V.splitAt (V.length fs `div` 2) fs


        toWLinesV :: (l, Seg p) -> ShadowFrontV l p
        toWLinesV (l, s@(Seg' aInit bInit)) = if isColinear
                then emptyV
                else if aS > bS
                    then case lineIntersection directedLine (line' focalPoint (V2 1 0)) of
                        LPoint mid -> ShadowFrontV . V.fromList $ catMaybes [
                                            WLine l (( 0, mid), (bS,   b)) <$> line mid (  b - mid),
                                            WLine l ((aS,   a), ( 4, mid)) <$> line   a (mid -   a)
                                        ]
                        _          -> emptyV
                    else ShadowFrontV $ V.singleton (WLine l ((aS, a), (bS, b)) directedLine)
...
data SVector a = SVector [a] (Vector a)

splitHead :: SVector a -> Maybe (a, SVector a)
splitHead (SVector [] v)
    | V.null v  = Nothing
    | otherwise = Just (V.head v, fromVector (V.tail v))
splitHead (SVector (a:as) v) = Just (a, SVector as v)

svCons :: a -> SVector a -> SVector a
svCons a (SVector as v) = SVector (a:as) v

fromVector :: Vector a -> SVector a
fromVector v = SVector [] v

svToList :: SVector a -> [a]
svToList (SVector as v) = as ++ V.toList v


joinSegsV :: ShadowFrontV l p -> ShadowFrontV l p
joinSegsV = id

mergeV :: forall p l. (Ord p, Fractional p, Num p, Show p, Show l)
      => Pos p
      -> ShadowFrontV l p
      -> ShadowFrontV l p
      -> ShadowFrontV l p
mergeV !focalPoint !(ShadowFrontV asV) !(ShadowFrontV bsV)
    = joinSegsV
    . ShadowFrontV
    . V.fromList
    . concatMap mergeWedge
    $ toWedges (fromVector asV) (fromVector bsV)
    where
        -- break into wedges
        toWedges :: SVector (WLine l p)
                 -> SVector (WLine l p)
        ...
        toWedges as' bs' = case (splitHead as', splitHead bs') of
            (Nothing, Nothing) -> []
            (_, Nothing) -> map (\c@(WLine cL cSSPs cLine) -> (cSSPs, Left (cL, cLine))) (svToList as')
            (Nothing, _) -> toWedges bs' as' -- flip
            (Just (a@(WLine aL aSSPs@((aS1,aSP1),(aS2,aSP2)) aLine), as),
             Just (b@(WLine bL bSSPs@((bS1,bSP1),(bS2,bSP2)) bLine), bs))
                -> case compare aS1 bS1 of
                    ...
                    EQ -> case compare aS2 bS2 of
                        ...
                        LT -> (aSSPs, Right ((aL, aLine), (bL, bLine))) : toWedges as ((WLine bL ((aS2,aSP2),(bS2,bSP2)) bLine) `svCons` bs)
                        -- b ends first -> consume b
                        GT -> (bSSPs, Right ((aL, aLine), (bL, bLine))) : toWedges ((WLine aL ((bS2,bSP2),(aS2,aSP2)) aLine) `svCons` as) bs
                    -- a starts before b
                    LT -> if aS2 <= bS1
                        -- a is entirely before b -> consume a
                        then (aSSPs, Left (aL, aLine)) : toWedges as bs'
                        -- a and b overlap -> consume a up to start of b
                        else (((aS1,aSP1),(bS1,bSP1)), Left (aL, aLine)) : toWedges ((WLine aL ((bS1,bSP1),(aS2,aSP2)) aLine) `svCons` as) bs'
                    -- b starts before a
                    GT -> if bS2 <= aS1
                        -- b is entirely before a -> consume b
                        then (bSSPs, Left (bL, bLine)) : toWedges as' bs
                        -- b and a overlap -> consume b up to start of a
                        else (((bS1,bSP1),(aS1,aSP1)), Left (bL, bLine)) : toWedges as' ((WLine bL ((aS1,aSP1),(bS2,bSP2)) bLine) `svCons` bs)
...
instance (NFData l, NFData p) => NFData (ShadowFront l p)

data WLine l p = WLine l ((p,Pos p), (p, Pos p)) (Line p)
    deriving (Show,Generic)

instance (NFData l, NFData p) => NFData (WLine l p)
...
{-# SPECIALIZE toShadowFront :: Show l => Pos Double -> Maybe [(l, Seg Double)] -> [(l, Seg Double)] -> ShadowFront l Double #-}
{-# SPECIALIZE toShadowFront :: Show l => Pos Float -> Maybe [(l, Seg Float)] -> [(l, Seg Float)] -> ShadowFront l Float #-}
...
toShadowFront focalPoint filterLabeledSegsMay labeledSegs
    ...
    $ labeledSegs
    where
        fromVToLShadowFront :: ShadowFront l p -> ShadowFront l p
        fromVToLShadowFront = undefined
...
{-# SPECIALIZE merge :: Show l => Pos Double -> ShadowFront l Double -> ShadowFront l Double -> ShadowFront l Double #-}
{-# SPECIALIZE merge :: Show l => Pos Float -> ShadowFront l Float -> ShadowFront l Float -> ShadowFront l Float #-}
