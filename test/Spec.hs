...
modelRayCast :: RayCast Double item =>
    Ray Double
    -> [item]
    -> [V2 Double]
modelRayCast ray@(Ray' rayStart _) items
    = sortOn (distSq rayStart)
    . mapMaybe (rayCast ray)
    $ items
...
tests = testGroup "Tests" [
        ...
        , testGroup "BVH" [
            testProperty "modelRayCast == BVH"
                (\ray (items :: [Seg Double]) ->
                    let
                        bvh = toBVH items
                    in modelRayCast ray items === map fst (rayCastMany ray bvh)
                )
        ]

        , testGroup "Ray-Box intersection" [

            testProperty "Box DOES intersect ray aimed at center" (\rayStart (box :: AABB TestType) ->
                    let
        ...
        , testGroup "Shadow fronts" [
              ...
              testCase "Single Seg" $
                let
                    ...
                    segs = [a]
                    expected = [a]
                in elem (shadowFrontToHasSegs focalPoint $ hasSegsToShadowFront focalPoint Nothing segs) (Utils.rotations expected) @?= True
        ...
        , testGroup "Filtered Shadow fronts" [
            ...
            testCase "1 Filter: part of seg + remove seg. " $
                ...
                in elem (shadowFrontToHasSegs focalPoint $ hasSegsToShadowFront focalPoint (Just filters) segs) (Utils.rotations expected) @?= True
...
atan2Positive :: RealFloat a => a -> a -> a
